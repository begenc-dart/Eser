...
import 'package:eser/src/core/local/key_value_storage_service.dart';
...
class ApiInterceptor extends Interceptor {
  ...
  Future<void> onRequest(
    ...
    if (options.extra.containsKey('requiresAuthToken')) {
      final token = await KeyValueStorageService().getAuthToken("token");
      log(token.toString());
      if (token.isNotEmpty) {
        options.headers.addAll(
          <String, Object?>{'Authorization': 'Bearer $token', "Content-Type": "application/json"},
        );
      }
